...
def get_country_mapping(conn):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT name, id, iso3 FROM country")
    
    # Create name → ID mapping
    mapping = {}
    for name, country_id, iso3 in cursor.fetchall():
        mapping[name] = country_id
        # Also map ISO3 if available
        if iso3:
            mapping[iso3] = country_id
...
def upsert_values(conn, country_mapping, data):
    ...
    for country_name, year, value in data:
        # Try to map country name → ISO3 → country_id
        ...
        country_id = country_mapping.get(iso3)
        if not country_id:
            unmapped_countries.add(country_name)
            ignored += 1
            continue
        ...
        cursor.execute("""
            ...
            WHERE id = %s AND country_id = %s AND year = %s
